    ...
    (() => {
                ...
                function setSpollerAction(e) {
                    ...
                    if (el.closest("[data-spoller]")) {
                        ...
                        if (!spollersBlock.querySelectorAll("._slide").length) {
                            ...
                            spollerTitle.parentElement.classList.toggle("active");
                        }
                        e.preventDefault();
                    }
                }
                ...
                if (tabsContent.length > 0) {
                    ...
                    tabsContent.forEach(((tabsContentItem, index) => {
                        if (tabsTitles[index].classList.contains("_tab-active")) {
                            if (tabsBlockAnimate) _slideDown(tabsContentItem, tabsBlockAnimate); else tabsContentItem.style.cssText = `display: block;\n\t\t\t\t\t\t`;
                            if (isHash && !tabsContentItem.closest(".popup")) setHash(`tab-${tabsBlockIndex}-${index}`);
                        } else if (tabsBlockAnimate) _slideUp(tabsContentItem, tabsBlockAnimate); else tabsContentItem.style.cssText = `display: none;\n\t\t\t\t\t`;
                    }));
                }
            }
        ...
        function initSliders() {
            if (document.querySelector(".sliders-product__slider")) new core(".sliders-product__slider", {
                ...
                navigation: {
                    prevEl: ".sliders-product__prev",
                    nextEl: ".sliders-product__next"
                },
                on: {}
            });
            if (document.querySelector(".firstscreen__slider")) new core(".firstscreen__slider", {
            ...
            if (document.querySelector(".novelties__slider")) new core(".novelties__slider", {
                modules: [ Navigation, Pagination, Autoplay ],
                observer: true,
                observeParents: true,
                slidesPerView: 4,
                ...
                navigation: {
                    prevEl: ".slider-novelties__prev",
                    nextEl: ".slider-novelties__next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1.5,
            ...
            resizableSwiper("(min-width: 479.98px)", ".categories__slider", {
                loop: true,
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
            ...
            resizableSwiper("(max-width: 767.98px)", ".usefull-information__slider", {
                loop: true,
                modules: [ Pagination ],
                observer: true,
                observeParents: true,
                ...
                spaceBetween: 5,
                pagination: {
                    el: ".useful-information__pagination",
                    clickable: true
                }
            }, someFunc);
            resizableSwiper("(min-width: 479.98px)", ".watched__slider", {
                loop: true,
                ...
                speed: 500,
                navigation: {
                    prevEl: ".slider-watched__prev",
                    nextEl: ".slider-watched__next"
                },
                autoplay: {
                    delay: 3e3,
                    disableOnInteraction: true
            ...
            resizableSwiper("(max-width: 767.98px)", ".image-slider", {
                modules: [ Pagination ],
                slidesPerView: 1,
                spaceBetween: 0,
                loop: true,
        ...
        var fixedGoTo = document.querySelector(".fixed-goto");
        var fixedCall = document.querySelector(".fixed-call");
        window.onscroll = function() {
            windowScroll();
        };
        function windowScroll() {
            fixedGoTo.classList.toggle("_active", fixedGoTo.scrollTop > 1e3 || document.documentElement.scrollTop > 1e3);
            fixedCall.classList.toggle("_active", fixedCall.scrollTop > 1e3 || document.documentElement.scrollTop > 1e3);
        }
        ...
        const productsBtn = document.querySelectorAll(".product-card__add-cart");
        ...
        const printQuantity = () => {
            ...
            let favoriteProductsListLength = favoriteProductsList.querySelector(".simplebar-content").children.length;
            favoriteQuantity.textContent = favoriteProductsListLength;
            favoriteProductsListLength > 0 ? favorite.classList.add("active") : favorite.classList.remove("active");
        };
        const printFullPrice = () => {
            fullPrice.textContent = `${normalPrice(price)} ₽`;
        };
        const generateCartProduct = (img, title, price, id) => `\n\t\t<li class="cart-content__item">\n\t\t\t<article class="cart-content__product cart-product" data-id="${id}">\n\t\t\t\t<a href="basket.html"><img src="${img}" alt="" class="cart-product__img"></a>\n\t\t\t\t<div class="cart-product__text">\n\t\t\t\t\t<h3 class="cart-product__title"><a href="basket.html">${title}</a></h3>\n\t\t\t\t\t<span class="cart-product__price">${normalPrice(price)}</span>\n\t\t\t\t</div>\n\t\t\t\t<button class="cart-product__delete _icon-delete" aria-label="Удалить товар"></button>\n\t\t\t</article>\n\t\t</li>\n\t`;
        const deleteProducts = productParent => {
            let id = productParent.querySelector(".cart-product").dataset.id;
            document.querySelector(`.product[data-id="${id}"]`).querySelector(".product-card__add-cart").disabled = false;
            document.querySelector(`.product[data-id="${id}"]`).querySelector(".product-card__add-cart").innerHTML = `В корзину`;
        ...
        productsBtn.forEach((el => {
            ...
            el.addEventListener("click", (e => {
                ...
                let priceNumber = parseInt(priceWithoutSpaces(parent.querySelector(".product-card__price-old").textContent));
                ...
                printQuantity();
                self.disabled = true;
                self.innerHTML = `<span>Добавлено</span>`;
            }));
        }));
        cartProductsList.addEventListener("click", (e => {
            if (e.target.classList.contains("cart-product__delete")) deleteProducts(e.target.closest(".cart-content__item"));
        }));
        const favoriteBtn = document.querySelectorAll(".product-card__add-favorite");
        const favoriteProductsList = document.querySelector(".favorite-content__list");
        const favorite = document.querySelector(".favorite");
        const favoriteQuantity = favorite.querySelector(".favorite__quantity");
        const favoriteFullPrice = document.querySelector(".fullprice");
        const favoriteNormalPrice = str => String(str).replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, "$1 ");
        const favoritePlusFullPrice = currentPrice => price += currentPrice;
        const favoriteMinusFullPrice = currentPrice => price -= currentPrice;
        const favoritePrintFullPrice = () => {
            favoriteFullPrice.textContent = `${favoriteNormalPrice(price)} ₽`;
        };
        const generateFavoriteProduct = (img, title, price, id) => `\n\t\t<li class="favorite-content__item">\n\t\t\t<article class="favorite-content__product favorite-product" data-id="${id}">\n\t\t\t\t<a href="favourites.html"><img src="${img}" alt="" class="favorite-product__img"></a>\n\t\t\t\t<div class="favorite-product__text">\n\t\t\t\t\t<h3 class="favorite-product__title"><a href="favourites.html">${title}</a></h3>\n\t\t\t\t\t<span class="favorite-product__price">${favoriteNormalPrice(price)}</span>\n\t\t\t\t</div>\n\t\t\t\t<button class="favorite-product__delete _icon-delete" aria-label="Удалить товар"></button>\n\t\t\t</article>\n\t\t</li>\n\t`;
        const favoriteDeleteProducts = productParent => {
            let id = productParent.querySelector(".favorite-product").dataset.id;
            document.querySelector(`.product[data-id="${id}"]`).querySelector(".product-card__add-favorite").disabled = false;
            document.querySelector(`.product[data-id="${id}"]`).querySelector(".product-card__add-favorite").classList.remove("active");
            if (document.querySelector(`.product[data-id="${id}"]`).querySelector(".header-product__favorite")) {
                document.querySelector(`.product[data-id="${id}"]`).querySelector(".header-product__favorite").innerHTML = `\n        <button\n                                        class="product-card__add-favorite header-product__favorite "\n                                        style="position: unset;">\n                                        <div class="header-product__svg">\n                                            <svg width="9" height="9" viewBox="0 0 9 9" fill="none"\n                                                xmlns="http://www.w3.org/2000/svg">\n                                                <g clip-path="url(#clip0_39_1700)">\n                                                    <path\n                                                        d="M8.07569 1.45491C7.63568 0.97768 7.03193 0.714844 6.37552 0.714844C5.88487 0.714844 5.43552 0.869965 5.03994 1.17586C4.84033 1.33027 4.65947 1.51918 4.5 1.73967C4.3406 1.51924 4.15967 1.33027 3.96 1.17586C3.56448 0.869965 3.11513 0.714844 2.62448 0.714844C1.96807 0.714844 1.36425 0.97768 0.924244 1.45491C0.489491 1.92656 0.25 2.57091 0.25 3.26934C0.25 3.9882 0.517895 4.64624 1.09305 5.34026C1.60757 5.96107 2.34705 6.59128 3.20339 7.32103C3.4958 7.57025 3.82725 7.85274 4.17141 8.15364C4.26233 8.23328 4.37899 8.27711 4.5 8.27711C4.62094 8.27711 4.73767 8.23328 4.82846 8.15377C5.17262 7.8528 5.50426 7.57019 5.7968 7.32084C6.65302 6.59121 7.3925 5.96107 7.90702 5.34019C8.48217 4.64624 8.75 3.9882 8.75 3.26928C8.75 2.57091 8.51051 1.92656 8.07569 1.45491Z"\n                                                        fill="#A6A6A6" />\n                                                </g>\n                                                <defs>\n                                                    <clipPath id="clip0_39_1700">\n                                                        <rect width="8.5" height="8.5" fill="white"\n                                                            transform="translate(0.25 0.25)" />\n                                                    </clipPath>\n                                                </defs>\n                                            </svg>\n                                        </div>\n                                        <span>Добавить в избранное</span>\n                                    </button>\n        `;
                document.querySelector(`.product[data-id="${id}"]`).querySelector(".header-product__favorite").classList.remove("active");
            }
            let currentPrice = parseInt(priceWithoutSpaces(productParent.querySelector(".favorite-product__price").textContent));
            favoriteMinusFullPrice(currentPrice);
            favoritePrintFullPrice();
            productParent.remove();
            printQuantity();
        };
        favoriteBtn.forEach((el => {
            ...
            el.addEventListener("click", (e => {
                ...
                let priceString = priceWithoutSpaces(parent.querySelector(".product-card__price").textContent);
                let priceNumber = parseInt(priceWithoutSpaces(parent.querySelector(".product-card__price-old").textContent));
                favoritePlusFullPrice(priceNumber);
                favoritePrintFullPrice();
                favoriteProductsList.querySelector(".simplebar-content").insertAdjacentHTML("afterbegin", generateFavoriteProduct(img, title, priceString, id));
                printQuantity();
                if (document.querySelector(`.product[data-id="${id}"]`).querySelector(".header-product__favorite")) {
                    document.querySelector(`.product[data-id="${id}"]`).querySelector(".header-product__favorite").innerHTML = `\n            <button\n                                        class="product-card__add-favorite header-product__favorite"\n                                        style="position: unset;">\n                                        <div class="header-product__svg">\n                                            <svg width="9" height="9" viewBox="0 0 9 9" fill="none"\n                                                xmlns="http://www.w3.org/2000/svg">\n                                                <g clip-path="url(#clip0_39_1700)">\n                                                    <path\n                                                        d="M8.07569 1.45491C7.63568 0.97768 7.03193 0.714844 6.37552 0.714844C5.88487 0.714844 5.43552 0.869965 5.03994 1.17586C4.84033 1.33027 4.65947 1.51918 4.5 1.73967C4.3406 1.51924 4.15967 1.33027 3.96 1.17586C3.56448 0.869965 3.11513 0.714844 2.62448 0.714844C1.96807 0.714844 1.36425 0.97768 0.924244 1.45491C0.489491 1.92656 0.25 2.57091 0.25 3.26934C0.25 3.9882 0.517895 4.64624 1.09305 5.34026C1.60757 5.96107 2.34705 6.59128 3.20339 7.32103C3.4958 7.57025 3.82725 7.85274 4.17141 8.15364C4.26233 8.23328 4.37899 8.27711 4.5 8.27711C4.62094 8.27711 4.73767 8.23328 4.82846 8.15377C5.17262 7.8528 5.50426 7.57019 5.7968 7.32084C6.65302 6.59121 7.3925 5.96107 7.90702 5.34019C8.48217 4.64624 8.75 3.9882 8.75 3.26928C8.75 2.57091 8.51051 1.92656 8.07569 1.45491Z"\n                                                        fill="#A6A6A6" />\n                                                </g>\n                                                <defs>\n                                                    <clipPath id="clip0_39_1700">\n                                                        <rect width="8.5" height="8.5" fill="white"\n                                                            transform="translate(0.25 0.25)" />\n                                                    </clipPath>\n                                                </defs>\n                                            </svg>\n                                        </div>\n                                        <span>В избранном</span>\n                                    </button>\n            `;
                    document.querySelector(`.product[data-id="${id}"]`).querySelector(".header-product__favorite").classList.add("active");
                }
                self.disabled = true;
                self.classList.add("active");
            }));
        }));
        favoriteProductsList.addEventListener("click", (e => {
            if (e.target.classList.contains("favorite-product__delete")) favoriteDeleteProducts(e.target.closest(".favorite-content__item"));
        }));
        let newsFilterButton = document.querySelectorAll(".page-news__button");
        let itemPointButton = document.querySelectorAll(".item-point__button");
        let butotnChoice = document.querySelectorAll(".form-basket__button_choice");
        let pointCurrent = document.querySelector(".point-current");
        let pointCurrentTitle = document.querySelector(".item-point__title").textContent;
        let pointCurrentName = document.querySelector(".item-point__name").textContent;
        let buttonPoint = document.querySelector(".button-point");
        const generatePickUpPoint = (pointCurrentName, pointCurrentTitle) => `\n    <div class="form-basket__point point-current">\n    <span class="item-point__title">${pointCurrentTitle}</span>\n    <span class="item-point__title">${pointCurrentName}</span>\n    </div>\n\t`;
        newsFilterButton.forEach((item => {
            item.addEventListener("click", (e => {
                newsFilterButton.forEach((el => {
                    el.classList.remove("_active");
                }));
                item.classList.add("_active");
            }));
        }));
        itemPointButton.forEach((item => {
            item.addEventListener("click", (e => {
                itemPointButton.forEach((el => {
                    el.classList.remove("_active");
                    el.innerHTML = `Выбрать этот пункт`;
                }));
                item.classList.add("_active");
                if (item.classList.contains("_active")) {
                    item.innerHTML = `Выбранный пункт`;
                    buttonPoint.innerHTML = `Выбранный пункт`;
                    buttonPoint.classList.add("_active");
                    pointCurrent.style.display = "grid";
                    pointCurrent.insertAdjacentHTML("afterbegin", generatePickUpPoint(pointCurrentTitle, pointCurrentName));
                }
            }));
        }));
        butotnChoice.forEach((item => {
            item.addEventListener("click", (e => {
                butotnChoice.forEach((el => {
                    el.classList.remove("_active");
                }));
                item.classList.add("_active");
            }));
